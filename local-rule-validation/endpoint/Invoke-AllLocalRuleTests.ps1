...
$runner = Join-Path $PSScriptRoot 'Invoke-LocalRuleTest.ps1'
...
$summary = [ordered]@{
    ...
    endpoint_log = $RunLog
    server_check_hint = './check-local-rule-results.sh --run-id "{0}" --manifest ./local_rule_tests.json --endpoint-results ./endpoint-results.jsonl' -f $RunId
}
...
Write-Host "Copy local_rule_tests.json and endpoint-results.jsonl to the Wazuh server, then run:"
